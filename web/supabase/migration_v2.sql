...
VALUES (
        'Media',
        'Media and audio/visual ministry'
    ),
    (
        'Praise And Worship',
    ...
    (
        'Dancers',
        'Dance and movement ministry'
    ),
    (
        'Ashers',
        'Ushering and hospitality ministry'
...
VALUES (
        'Member',
        'Basic church member',
        '["view_dashboard","view_profile","update_profile","change_password","view_schedule","view_events","view_announcements","view_community","create_posts","like_comment_posts","view_messages","send_messages","rsvp_events","rsvp_meetings","view_meetings","view_reports","logout"]'
    ),
    (
        'Song Leader',
        'Praise and worship song leader',
        '["view_dashboard","view_profile","update_profile","change_password","view_schedule","view_events","view_announcements","view_community","create_posts","like_comment_posts","view_messages","send_messages","send_updates_pastors","rsvp_events","rsvp_meetings","view_meetings","create_meetings","update_meetings","create_lineup","update_lineup","assign_singers","view_reports","view_ministry_reports","logout"]'
    ),
    (
        'Leader',
        'Ministry leader',
    ...
    (
        'Pastor',
        'Church pastor',
        '["view_pastor_dashboard","view_analytics","view_bible_verses","view_all_ministries","view_ministry_members","assign_members","create_schedule","update_schedule","create_events","update_events","delete_events","view_event_participants","create_announcements","view_announcements","update_announcements","delete_announcements","view_attendance","mark_attendance","update_attendance","delete_attendance","send_broadcasts","view_community_extended","pin_posts","view_ministry_updates","generate_reports","view_profile","update_profile","logout"]'
    ),
    (
        'Admin',
        'System administrator',
        '["view_admin_dashboard","view_system_analytics","view_bible_verses","create_users","view_users","update_users","deactivate_users","activate_users","reset_passwords","create_ministry","view_ministry","update_ministry","delete_ministry","assign_users_ministry","create_events","view_events","update_events","delete_events","create_announcements","view_announcements","update_announcements","delete_announcements","view_attendance","mark_attendance","update_attendance","delete_attendance","generate_reports","view_reports","export_reports","send_broadcasts","view_profile","update_profile","logout"]'
    ),
    (
        'Super Admin',
        'Full system access',
...
VALUES (
        'church_name',
        '"SanctuaryHub"',
        'Church name displayed across the portal'
    ),
    (
        'bible_verse_api',
        '"groq"',
        'Bible verse API provider'
    ),
    (
        'theme_primary_color',
        '"#926C15"',
...
CREATE POLICY "Allow all" ON event_rsvps FOR ALL USING (true)
WITH
    CHECK (true);

CREATE POLICY "Allow all" ON ministry_meetings FOR ALL USING (true)
WITH
    CHECK (true);

CREATE POLICY "Allow all" ON meeting_rsvps FOR ALL USING (true)
WITH
    CHECK (true);
...
CREATE POLICY "Allow all" ON community_posts FOR ALL USING (true)
WITH
    CHECK (true);

CREATE POLICY "Allow all" ON post_likes FOR ALL USING (true)
WITH
    CHECK (true);

CREATE POLICY "Allow all" ON post_comments FOR ALL USING (true)
WITH
    CHECK (true);
...
CREATE POLICY "Allow all" ON ministries FOR ALL USING (true)
WITH
    CHECK (true);
