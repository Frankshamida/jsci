...
function handleSignup(data) {
  try {
    ...
    // Build the new row with security questions (Columns K and L)
    const newRow = [
      timestamp,                    // A - Timestamp
      ...
      data.ministry || '',         // F - Ministry
      data.username,               // G - Username
      hashedPassword,              // H - Password Hash
      hashedConfirmPassword,       // I - Confirm Password Hash
      data.terms ? 'Accepted' : 'Not Accepted', // J - Terms Accepted
      status,                      // K - Status
      data.securityQuestion || '', // L - Security Question (Column K)
      data.securityAnswer || ''    // M - Security Answer (Column L)
    ];
...
// security.js - Forgot Password Security Questions Functions
...
function getSecurityQuestion(username) {
  try {
    ...
    for (var i = 1; i < values.length; i++) {
      ...
      if (currentUsername && currentUsername.toString().toLowerCase() === username.toLowerCase()) {
...
function verifySecurityAnswer(data) {
  try {
    ...
    Logger.log('Verifying security answer for:', data.username);
    
    for (var i = 1; i < values.length; i++) {
      const row = values[i];
      const currentUsername = row[6]; // Column G (username)
      
      if (currentUsername && currentUsername.toString().toLowerCase() === data.username.toLowerCase()) {
        const storedAnswer = row[12] || ''; // Column M (Security Answer)
        
        Logger.log('Stored answer:', storedAnswer);
        Logger.log('Provided answer:', data.answer);
        ...
        if (storedAnswer.toString().toLowerCase().trim() === data.answer.toLowerCase().trim()) {
...
function resetPassword(data) {
  try {
    ...
    Logger.log('Resetting password for:', data.username);
    ...
    for (var i = 1; i < values.length; i++) {
      ...
      if (currentUsername && currentUsername.toString().toLowerCase() === data.username.toLowerCase()) {
        ...
        Logger.log('Password reset successfully for user:', data.username);
...
////////////////////FINALLL APPSCRIPT FILESSSSSSSSSSSSSSSSSSSSS ////////////////////////////////
